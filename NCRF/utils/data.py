...
class Data:
    def __init__(self):
        ...
        self.path_reduced_tagset = None
        self.disco_encoder= None
        self.add_leaf_unary_column = None      
        self.log_file=None  
        self.evalb_param_file=None
        self.dummy_os=True
        self.label_split_char='{}'
        self.disco_decode_script=None
        
        ### BertEncoder hyperparameters
        self.HP_transformer_num_attention_heads=2
        self.HP_transformer_num_hidden_layers=2
        self.HP_transformer_intermediate_size=800
        self.HP_transformer_hidden_dropout_prob = 0.1
    ...
    def show_data_summary(self):
        ...
        if self.word_feature_extractor.upper()=="TRANSFORMER":
            print("     Hyper      transformer_intermediate_size: %s" % (self.HP_transformer_intermediate_size))
            print("     Hyper      transformer_num_attention_heads: %s" % (self.HP_transformer_num_attention_heads))
            print("     Hyper      transformer_layer: %s" % (self.HP_transformer_num_hidden_layers))
            print("     Hyper      transformer_hidden_dropout_prob: %s" % (self.HP_transformer_hidden_dropout_prob))
    ...
    def read_config(self, config_file):
        ...
        ###BertEncoder hyperparameters###
        the_item='transformer_num_attention_heads'
        if the_item in config:
            
            self.HP_transformer_num_attention_heads=int(config[the_item])
        the_item='transformer_num_hidden_layers'
        if the_item in config:
            self.HP_transformer_num_hidden_layers=int(config[the_item])
            
        the_item='transformer_intermediate_size'
        if the_item in config:
            self.HP_transformer_intermediate_size=int(config[the_item])
    
        the_item = 'transformer_hidden_dropout_prob'
        if the_item in config:
            self.HP_transformer_hidden_dropout_prob=float(config[the_item])
    
        ### Hyperparameters related to multi-task learning ###
        ...
        the_item = "evalb_param_file"
        if the_item in config:
            self.evalb_param_file = config[the_item]

        the_item = "disco_decode_script"
        if the_item in config:
            self.disco_decode_script = config[the_item]
            
        the_item = "disco_encoder"
        if the_item in config:
            self.disco_encoder = config[the_item].lower()
            
        the_item = "add_leaf_unary_column"
        if the_item in config:
            self.add_leaf_unary_column = str2bool(config[the_item])
            
        the_item = "path_reduced_tagset"
        if the_item in config:
            self.path_reduced_tagset = config[the_item]
            
        the_item = "log_file"
        if the_item in config:
            self.log_file = config[the_item]
            
        the_item = "dummy_os"
        if the_item in config:
            self.dummy_os= str2bool(config[the_item])
            
        the_item = "label_split_char"
        if the_item in config:
            self.label_split_char= config[the_item]
